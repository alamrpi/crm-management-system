...
<div class="card-body py-0">
    <div class="card card-animate border border-1 mb-1">
        <div class="card-body">
            <div class="row">
                <div class="col-12">
                    <a href="javascript:void(0)"><h5 class="fs-14 fw-semibold cursor-pointer">{{ $task->task_name }}</h5></a>
                </div>
            </div>
            <div class="row">
                <div class="col-4">
                    <div class="d-flex align-items-center" style="width: 100px;">
                        <div class="flex-shrink-0 me-1 text-muted fs-10">{{ \App\Utility\ProjectTaskHelper::getTaskCompleteRatioByTaskId($task->id) }}%</div>
                        <div class="progress progress-sm  flex-grow-1" style="width: 68%;">
                            <div class="progress-bar bg-primary rounded" role="progressbar" style="width:{{ \App\Utility\ProjectTaskHelper::getTaskCompleteRatioByTaskId($task->id) }}%" aria-valuenow="{{ \App\Utility\ProjectTaskHelper::getTaskCompleteRatioByTaskId($task->id) }}" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>
                    </div>
                    <p class="text-muted fs-12">{{ \App\Utility\ProjectTaskHelper::getCompletedHourByTaskId($task->id) }}/{{ \App\Utility\ProjectTaskHelper::getTotalHourByTaskId($task->id) }} Hrs</p>
                </div>
